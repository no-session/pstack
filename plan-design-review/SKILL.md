...
### The pstack designer — YOUR PRIMARY TOOL

You have the **pstack designer**, an AI mockup generator that creates real visual mockups
...
MUST be saved to `~/.pstack/projects/$SLUG/designs/`, NEVER to `.context/`,
...
pstack designer is available (`DESIGN_READY` was printed during setup), **generate
mockups immediately.** Do not ask permission. This is the default behavior.

Tell the user: "Generating visual mockups with the pstack designer. This is how we
...
`~/.pstack/projects/$SLUG/designs/` (user config directory, not project files).
Mockups are design artifacts that inform the plan, not code changes. The pstack
...
- `mkdir -p ~/.pstack/projects/$SLUG/designs/...`
...
**If `DESIGN_NOT_AVAILABLE`:** Tell the user: "The pstack designer isn't set up yet. Run `$D setup` to enable visual mockups. Proceeding with text-only review, but you're missing the best part." Then proceed to review passes with text-based review.
...
**Log the result:**
```bash
~/.claude/skills/pstack/bin/pstack-review-log '{"skill":"design-outside-voices","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
$D generate --brief "<description of what 10/10 looks like for this dimension>" --output /tmp/pstack-ideal-<dimension>.png
```
...
Source: [OpenAI "Designing Delightful Frontends with GPT-5.4"](https://developers.openai.com/blog/designing-delightful-frontends-with-gpt-5-4) (Mar 2026) + pstack design methodology.
...
|----------------|-------------|-----------|-------|
| [screen name]  | ~/.pstack/projects/$SLUG/designs/[folder]/[filename].png | [brief description] | [constraints from review] |
```
...
```bash
~/.claude/skills/pstack/bin/pstack-review-log '{"skill":"plan-design-review","timestamp":"TIMESTAMP","status":"STATUS","initial_score":N,"overall_score":N,"unresolved":N,"decisions_made":N,"commit":"COMMIT"}'
```
