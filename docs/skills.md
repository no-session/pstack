...
Detailed guides for every pstack skill — philosophy, workflow, and examples.
...
| [`/pstack-upgrade`](#pstack-upgrade) | **Self-Updater** | Upgrade pstack to the latest version. Detects global vs vendored install, syncs both, shows what changed. |
...
Visions and decisions are persisted to `~/.pstack/projects/` so they survive beyond the conversation. Exceptional visions can be promoted to `docs/designs/` in your repo for the team.
...
Eng Review is the only required gate (disable with `pstack-config set skip_eng_review true`). CEO and Design are informational — recommended for product and UI changes respectively.

### Plan-to-QA flow

When `/plan-eng-review` finishes the test review section, it writes a test plan artifact to `~/.pstack/projects/`. When you later run `/qa`, it picks up that test plan automatically — your engineering review feeds directly into QA testing with no manual copy-paste.
...
Claude: [Runs full 80-item visual audit on the live site]
        ...
        [Report with before/after screenshots saved to .pstack/design-reports/]
```
...
Claude: [Explores 12 pages, fills 3 forms, tests 2 flows]
        ...
        [Full report with screenshots saved to .pstack/qa-reports/]
```
...
Keep pstack current with one command. It detects your install type (global at `~/.claude/skills/pstack` vs vendored in your project at `.claude/skills/pstack`), runs the upgrade, syncs both copies if you have dual installs, and shows you what changed.
...
Set `auto_upgrade: true` in `~/.pstack/config.yaml` to skip the prompt entirely — pstack upgrades silently at the start of each session when a new version is available.
...
Install Greptile on your GitHub repo at [greptile.com](https://greptile.com) — it takes about 30 seconds. Once it's reviewing your PRs, pstack picks up its comments automatically. No additional configuration.
...
pstack solves this. `/review` and `/ship` are now Greptile-aware. They read Greptile's comments, classify each one, and take action:
