# Design: pstack Visual Design Generation (`design` binary)

Generated by /validate on 2026-03-26
...
A compiled TypeScript binary (`design/dist/design`) that wraps the OpenAI Images/Responses API, callable from skill templates via `$D` (mirroring the existing `$B` browse binary pattern). Priority integration order: /validate → /plan-design-review → /design-consultation → /design-review.
...
4. Priority: /validate first, then /plan-design-review, /design-consultation, /design-review.
...
- Scoped 48-hour prototype: shared `visual_mockup.ts` utility, /validate + /plan-design-review only, hero mockup + 2 variants
...
**Screenshot consent (first-time only for $D evolve):** "This will send a screenshot of your live site to OpenAI for design evolution. [Proceed] [Don't ask again]" Stored in ~/.pstack/config.yaml as design_screenshot_consent.
...
- Exploration variants go to `/tmp/pstack-mockups-{session}/` (ephemeral, not committed)
...
- Fallback: if not in a git repo, save to `/tmp/pstack-mockup-{timestamp}.png`
...
```
Note: Codex runtime setup (`setup` script) must also export `PSTACK_DESIGN` env var, similar to how `PSTACK_BROWSE` is set.
...
**1. /validate** — Replace the Visual Sketch section
...
| `scripts/resolvers/preamble.ts` | Add `PSTACK_DESIGN` env var export for Codex host |
...
| `validate/SKILL.md.tmpl` | Replace Visual Sketch section with `{{DESIGN_MOCKUP}}` |
...
1. Read `~/.pstack/openai.json` → `{ "api_key": "sk-..." }` (file permissions 0600)
...
3. If neither exists → guided setup flow:
   ...
   - Write to `~/.pstack/openai.json` with 0600 permissions
...
- Running `/validate` on a UI idea produces actual PNG mockups alongside the design doc
...
- Symlinked via existing `~/.claude/skills/pstack/` install path
...
- Auth module (read ~/.pstack/openai.json, fallback to env var, guided setup flow)
...
- Add PSTACK_DESIGN env var export to `scripts/resolvers/preamble.ts` (Codex host)
- Update `test/gen-skill-docs.test.ts` (DESIGN_SKETCH test suite)
- Regenerate SKILL.md files

### Commit 4: /validate integration
...
5. Running `/validate` on a UI idea produces mockups inline
...
- You prioritized /validate first — the upstream leverage point. If the brainstorm produces real mockups, every downstream skill (/plan-design-review, /design-review) has a visual artifact to reference instead of re-interpreting prose.
...
|--------|---------|-----|------|--------|----------|
| Office Hours | `/validate` | Design brainstorm | 1 | DONE | 4 premises, 1 revised (Codex: opt-in->default-on) |
