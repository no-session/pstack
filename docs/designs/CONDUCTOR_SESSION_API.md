...
When Claude controls your real browser via CDP (pstack `$B connect`), you look at two
windows: **Conductor** (to see Claude's thinking) and **Chrome** (to see Claude's actions).

pstack's Chrome extension Side Panel shows browse activity — every command, result,
...
A "Session" tab in the pstack Chrome extension Side Panel that shows:
  ...
  "workspaces": [
    {
      "id": "abc123",
      "name": "pstack",
      "branch": "pawan/chrome-extension-ctrl",
...
## What pstack builds (extension side)
