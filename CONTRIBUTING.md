...
Thanks for wanting to make pstack better. Whether you're fixing a typo in a skill prompt or building an entirely new workflow, this guide will get you up and running fast.

## Quick start

pstack skills are Markdown files that Claude Code discovers from a `skills/` directory. Normally they live at `~/.claude/skills/pstack/` (your global install). But when you're developing pstack itself, you want Claude Code to use the skills *in your working tree* — so edits take effect instantly without copying or deploying anything.

That's what dev mode does. It symlinks your repo into the local `.claude/skills/` directory so Claude Code reads skills straight from your checkout.

```bash
git clone <repo> && cd pstack
...
Contributor mode turns pstack into a self-improving tool. Enable it and Claude Code
will periodically reflect on its pstack experience — rating it 0-10 at the end of
each major workflow step. When something isn't a 10, it thinks about why and files
a report to `~/.pstack/contributor-logs/` with what happened, repro steps, and what
would make it better.

```bash
~/.claude/skills/pstack/bin/pstack-config set pstack_contributor true
```

The logs are for **you**. When something bugs you enough to fix, the report is
already written. Fork pstack, symlink your fork into the project where you hit
the issue, fix it, and open a PR.

### The contributor workflow

1. **Use pstack normally** — contributor mode reflects and logs issues automatically
2. **Check your logs:** `ls ~/.pstack/contributor-logs/`
3. **Fork and clone pstack** (if you haven't already)
4. **Symlink your fork into the project where you hit the bug:**
   ```bash
   # In your core project (the one where pstack annoyed you)
   ln -sfn /path/to/your/pstack-fork .claude/skills/pstack
   cd .claude/skills/pstack && bun install && bun run build && ./setup
   ```
   Setup creates the per-skill symlinks (`qa -> pstack/qa`, etc.) and asks your
   prefix preference. Pass `--no-prefix` to skip the prompt and use short names.
5. **Fix the issue** — your changes are live immediately in this project
6. **Test by actually using pstack** — do the thing that annoyed you, verify it's fixed
7. **Open a PR from your fork**

This is the best way to contribute: fix pstack while doing your real work, in the
project where you actually felt the pain.

### Session awareness

When you have 3+ pstack sessions open simultaneously, every question tells you which project, which branch, and what's happening. No more staring at a question thinking "wait, which window is this?" The format is consistent across all skills.

## Working on pstack inside the pstack repo

When you're editing pstack skills and want to test them by actually using pstack
...
```
pstack/                          <- your working tree
├── .claude/skills/              <- created by dev-setup (gitignored)
│   ├── pstack -> ../../         <- symlink back to repo root
│   ├── review -> pstack/review  <- short names (default)
│   ├── ship -> pstack/ship      <- or pstack-review, pstack-ship if --prefix
...
Skill symlink names depend on your prefix setting (`~/.pstack/config.yaml`).
Short names (`/review`, `/ship`) are the default. Run `./setup --prefix` if you
prefer namespaced names (`/pstack-review`, `/pstack-ship`).
...
When E2E tests run, they produce machine-readable artifacts in `~/.pstack-dev/`:
...
Artifacts are never cleaned up — they accumulate in `~/.pstack-dev/` for post-mortem debugging and trend analysis.
...
pstack generates SKILL.md files for two hosts: **Claude** (`.claude/skills/`) and **Codex** (`.agents/skills/`). Every template change needs to be generated for both.
...
| Aspect | Claude | Codex |
|--------|--------|-------|
| Output directory | `{skill}/SKILL.md` | `.agents/skills/pstack-{skill}/SKILL.md` (generated at setup, gitignored) |
| Frontmatter | Full (name, description, allowed-tools, hooks, version) | Minimal (name + description only) |
| Paths | `~/.claude/skills/pstack` | `$PSTACK_ROOT` (`.agents/skills/pstack` in a repo, otherwise `~/.codex/skills/pstack`) |
...
**This is the recommended way to develop pstack.** Symlink your pstack checkout
...
```bash
# In your core project (not the pstack repo)
ln -sfn /path/to/your/pstack-checkout .claude/skills/pstack
```

### Step 2: Run setup to create per-skill symlinks

The `pstack` symlink alone isn't enough. Claude Code discovers skills through
individual symlinks (`qa -> pstack/qa`, `ship -> pstack/ship`, etc.), not through
the `pstack/` directory itself. Run `./setup` to create them:

```bash
cd .claude/skills/pstack && bun install && bun run build && ./setup
```

Setup will ask whether you want short names (`/qa`) or namespaced (`/pstack-qa`).
Your choice is saved to `~/.pstack/config.yaml` and remembered for future runs.
...
Remove the project-local symlink. Claude Code falls back to `~/.claude/skills/pstack/`:

```bash
rm .claude/skills/pstack
```

The per-skill symlinks (`qa`, `ship`, etc.) still point to `pstack/...`, so they'll
resolve to the global install automatically.

### Switching prefix mode

If you vendored pstack with one prefix setting and want to switch:

```bash
cd .claude/skills/pstack && ./setup --no-prefix   # switch to /qa, /ship
cd .claude/skills/pstack && ./setup --prefix       # switch to /pstack-qa, /pstack-ship
```
