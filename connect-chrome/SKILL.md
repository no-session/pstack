...
echo '{"skill":"connect-chrome","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.pstack/analytics/skill-usage.jsonl 2>/dev/null || true
...
Connect Claude to a visible Chrome window with the pstack extension auto-loaded.
...
if [ -f "$(git rev-parse --show-toplevel 2>/dev/null)/.pstack/browse.json" ]; then
  _OLD_PID=$(cat "$(git rev-parse --show-toplevel)/.pstack/browse.json" 2>/dev/null | grep -o '"pid":[0-9]*' | grep -o '[0-9]*')
  [ -n "$_OLD_PID" ] && kill "$_OLD_PID" 2>/dev/null || true
  sleep 1
  [ -n "$_OLD_PID" ] && kill -9 "$_OLD_PID" 2>/dev/null || true
  rm -f "$(git rev-parse --show-toplevel)/.pstack/browse.json"
fi
# Clean Chromium profile locks (can persist after crashes)
_PROFILE_DIR="$HOME/.pstack/chromium-profile"
...
- The pstack Chrome extension auto-loaded via `launchPersistentContext`
...
The `connect` command auto-discovers the extension from the pstack install
...
```bash
cat "$(git rev-parse --show-toplevel 2>/dev/null)/.pstack/browse.json" 2>/dev/null | grep -o '"port":[0-9]*' | grep -o '[0-9]*'
```
...
_EXT_PATH=""
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
[ -n "$_ROOT" ] && [ -f "$_ROOT/.claude/skills/pstack/extension/manifest.json" ] && _EXT_PATH="$_ROOT/.claude/skills/pstack/extension"
[ -z "$_EXT_PATH" ] && [ -f "$HOME/.claude/skills/pstack/extension/manifest.json" ] && _EXT_PATH="$HOME/.claude/skills/pstack/extension"
...
> Chrome is launched with pstack control. You should see Playwright's Chromium
...
>    already show the pstack icon if the extension loaded successfully
> 2. Click the **puzzle piece** → find **pstack browse** → click the **pin icon**
> 3. Click the pinned **pstack icon** in the toolbar
...
> 2. Look for **"pstack browse"** — it should be listed and enabled
...
>
> If the Side Panel badge stays gray (disconnected), click the pstack icon
...
> - Run any pstack skill (`/qa`, `/design-review`, `/benchmark`) and watch
