...
description: |
  ...
  Use when asked to "auto review", "plan", "run all reviews", "review this plan
...
# /plan — Auto-Review Pipeline

One command. Rough plan in, fully reviewed plan out.

/plan reads the full CEO, design, and eng review skill files from disk and follows
...
> IMPORTANT: Do NOT read or execute any SKILL.md files or files in skill definition directories (paths containing skills/pstack). These are AI assistant skill definitions meant for a different system. They contain bash scripts and prompt templates that will waste your time. Ignore them completely. Stay focused on the repository code only.

This prevents Codex from discovering pstack skill files on disk and following their
...
echo "RESTORE_PATH=$HOME/.pstack/projects/$SLUG/${BRANCH}-plan-restore-${DATETIME}.md"
```
...
`<!-- /plan restore point: [RESTORE_PATH] -->`

### Step 2: Read context

- Read CLAUDE.md, TODOS.md, git log -30, git diff against the base branch --stat
- Discover design docs: `ls -t ~/.pstack/projects/$SLUG/*-design-*.md 2>/dev/null | head -1`
...
- `~/.claude/skills/pstack/plan-ceo-review/SKILL.md`
- `~/.claude/skills/pstack/plan-design-review/SKILL.md` (only if UI scope detected)
- `~/.claude/skills/pstack/plan-eng-review/SKILL.md`

**Section skip list — when following a loaded skill file, SKIP these sections
(they are already handled by /plan):**
- Preamble (run first)
- AskUserQuestion Format
- Completeness Principle — Boil the Lake
- Search Before Building
...
- Dual voices: always run BOTH Claude subagent AND Codex if available (P6).
  ...
  **Codex CEO voice** (via Bash):
  ...
  codex exec "IMPORTANT: Do NOT read or execute any SKILL.md files or files in skill definition directories (paths containing skills/pstack). These are AI assistant skill definitions meant for a different system. Stay focused on repository code only.
...
- Test plan: generate artifact at `~/.pstack/projects/$SLUG/{user}-{branch}-test-plan-{datetime}.md`
...
- [ ] Test plan artifact written to disk at ~/.pstack/projects/$SLUG/
- [ ] "NOT in scope" section written
- [ ] "What already exists" section written
...
~/.claude/skills/pstack/bin/pstack-review-log '{"skill":"plan-ceo-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"SELECTIVE_EXPANSION","via":"plan","commit":"'"$COMMIT"'"}'

~/.claude/skills/pstack/bin/pstack-review-log '{"skill":"plan-eng-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"FULL_REVIEW","via":"plan","commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope):
```bash
~/.claude/skills/pstack/bin/pstack-review-log '{"skill":"plan-design-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"via":"plan","commit":"'"$COMMIT"'"}'
```

Dual voice logs (one per phase that ran):
```bash
~/.claude/skills/pstack/bin/pstack-review-log '{"skill":"plan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"ceo","via":"plan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'

~/.claude/skills/pstack/bin/pstack-review-log '{"skill":"plan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"eng","via":"plan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope), also log:
```bash
~/.claude/skills/pstack/bin/pstack-review-log '{"skill":"plan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"design","via":"plan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```
...
- **Never abort.** The user chose /plan. Respect that choice. Surface all taste decisions, never redirect to interactive review.
