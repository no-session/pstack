...
function showStatusPill(connected, refs) {
  ...
  if (!statusPill) {
    ...
    statusPill.id = 'pstack-status-pill';
  ...
  statusPill.innerHTML = `<span class="pstack-pill-dot"></span> pstack${refText}`;
...
function ensureContainer() {
  ...
  overlayContainer.id = 'pstack-ref-overlays';
...
function renderRefBadges(refs) {
  ...
  for (const ref of refs) {
    ...
    badge.className = 'pstack-ref-badge';
...
function renderRefPanel(refs) {
  ...
  panel.className = 'pstack-ref-panel';

  const header = document.createElement('div');
  header.className = 'pstack-ref-panel-header';
  header.textContent = `pstack refs (${refs.length})`;
  header.style.cssText = 'pointer-events: auto; cursor: move;';
  panel.appendChild(header);

  const list = document.createElement('div');
  list.className = 'pstack-ref-panel-list';
  for (const ref of refs.slice(0, 30)) { // Show max 30 in panel
    const row = document.createElement('div');
    row.className = 'pstack-ref-panel-row';
    const idSpan = document.createElement('span');
    idSpan.className = 'pstack-ref-panel-id';
    idSpan.textContent = ref.ref;
    const roleSpan = document.createElement('span');
    roleSpan.className = 'pstack-ref-panel-role';
    roleSpan.textContent = ref.role;
    const nameSpan = document.createElement('span');
    nameSpan.className = 'pstack-ref-panel-name';
  ...
  if (refs.length > 30) {
    ...
    more.className = 'pstack-ref-panel-more';
