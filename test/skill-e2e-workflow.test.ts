...
describeIfSelected('Ship workflow E2E', ['ship-local-workflow'], () => {
  ...
  beforeAll(() => {
    shipRemoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pstack-ship-remote-'));
    shipWorkDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pstack-ship-work-'));
...
describeIfSelected('pstack-upgrade E2E', ['pstack-upgrade-happy-path'], () => {
  ...
  testConcurrentIfSelected('pstack-upgrade-happy-path', async () => {
    const mockPstack = path.join(upgradeDir, '.claude', 'skills', 'pstack');
    const result = await runSkillTest({
...
IMPORTANT: The install directory is at ./.claude/skills/pstack — use that exact path.`,
    ...
    recordE2E(evalCollector, '/pstack-upgrade happy path', 'pstack-upgrade E2E', result, {
