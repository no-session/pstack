...
5. Write a deploy report skeleton to .pstack/deploy-reports/report.md showing the
...
Do NOT use AskUserQuestion. Do NOT run gh or fly commands.`,
    ...
    const reportDir = path.join(landDir, '.pstack', 'deploy-reports');
...
5. Write the dry-run report to .pstack/deploy-reports/dry-run-validation.md
...
Just demonstrate the first-run dry-run output.`,
    ...
    const reportDir = path.join(firstRunDir, '.pstack', 'deploy-reports');
    expect(fs.existsSync(reportDir)).toBe(true);
...
(pstack-review-read would return NO_REVIEWS).

Simulate what the readiness gate would show:
1. Run pstack-review-read equivalent (simulate NO_REVIEWS output)
2. Determine review staleness: Eng Review should be "NOT RUN"
3. Note that Step 3.5a-bis would offer an inline review
4. Write a simulated readiness report to .pstack/deploy-reports/readiness-report.md
...
Show what the readiness gate output would look like.`,
    ...
    const reportDir = path.join(reviewDir, '.pstack', 'deploy-reports');
    expect(fs.existsSync(reportDir)).toBe(true);
...
Instead, demonstrate you understand the workflow:
1. Create the .pstack/benchmark-reports/ directory structure including baselines/
2. Write a simulated baseline.json to .pstack/benchmark-reports/baselines/baseline.json
...
3. Write a simulated benchmark report to .pstack/benchmark-reports/benchmark-report.md
...
Just create the files showing the correct schema and report format.`,
    ...
    expect(fs.existsSync(path.join(benchDir, '.pstack', 'benchmark-reports'))).toBe(true);
    const baselineDir = path.join(benchDir, '.pstack', 'benchmark-reports', 'baselines');
