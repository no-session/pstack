...
const ALL_SKILLS = (() => {
  ...
  if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
    skills.push({ dir: '.', name: 'root pstack' });
  }
...
describe('gen-skill-docs', () => {
  ...
  test('generated SKILL.md contains contributor mode check', () => {
    ...
    expect(content).toContain('pstack_contributor');
  ...
  test('preamble .pending-* glob is zsh-safe (uses find, not shell glob)', () => {
    for (const skill of ALL_SKILLS) {
      ...
      expect(content).toContain("find ~/.pstack/analytics -maxdepth 1 -name '.pending-*'");
    }
  });
...
describe('REVIEW_DASHBOARD resolver', () => {
  ...
  for (const skill of REVIEW_SKILLS) {
    test(`review dashboard appears in ${skill} generated file`, () => {
      ...
      expect(content).toContain('pstack-review');
...
describe('CODEX_SECOND_OPINION resolver', () => {
  ...
  test('Codex host variant does NOT contain the Phase 3.5 resolver output', () => {
    ...
    expect(codexContent).not.toContain('pstack-codex-oh-');
  });
});
...
describe('Codex filesystem boundary', () => {
  ...
  test('codex skill has rabbit-hole detection rule', () => {
    ...
    expect(content).toContain('pstack-update-check');
  ...
  test('autoplan boundary text avoids host-specific paths for cross-host compatibility', () => {
    ...
    expect(boundarySection).toContain('skills/pstack');
...
describe('Codex generation (--host codex)', () => {
  ...
  const CODEX_SKILLS = (() => {
    ...
    if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
      if (!isSymlinkLoop('pstack')) {
        skills.push({ dir: '.', codexName: 'pstack' });
      }
    }
    for (const entry of fs.readdirSync(ROOT, { withFileTypes: true })) {
      if (!entry.isDirectory() || entry.name.startsWith('.') || entry.name === 'node_modules') continue;
      ...
      const codexName = entry.name.startsWith('pstack-') ? entry.name : `pstack-${entry.name}`;
  ...
  test('root pstack bundle has OpenAI metadata for Codex skill browsing', () => {
    ...
    expect(content).toContain('display_name: "pstack"');
    expect(content).toContain('Use $pstack to locate the bundled pstack skills.');
    expect(content).toContain('allow_implicit_invocation: true');
  });

  test('codexSkillName mapping: root is pstack, others are pstack-{dir}', () => {
    // Root → pstack
    expect(fs.existsSync(path.join(AGENTS_DIR, 'pstack', 'SKILL.md'))).toBe(true);
    // Subdirectories → pstack-{dir}
    expect(fs.existsSync(path.join(AGENTS_DIR, 'pstack-review', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(AGENTS_DIR, 'pstack-ship', 'SKILL.md'))).toBe(true);
    // pstack-upgrade doesn't double-prefix
    expect(fs.existsSync(path.join(AGENTS_DIR, 'pstack-upgrade', 'SKILL.md'))).toBe(true);
    // No double-prefix: pstack-pstack-upgrade must NOT exist
    expect(fs.existsSync(path.join(AGENTS_DIR, 'pstack-pstack-upgrade', 'SKILL.md'))).toBe(false);
  });
  ...
  test('/codex skill excluded from Codex output', () => {
    expect(fs.existsSync(path.join(AGENTS_DIR, 'pstack-codex', 'SKILL.md'))).toBe(false);
    expect(fs.existsSync(path.join(AGENTS_DIR, 'pstack-codex'))).toBe(false);
  });

  test('Codex review step stripped from Codex-host ship and review', () => {
    const shipContent = fs.readFileSync(path.join(AGENTS_DIR, 'pstack-ship', 'SKILL.md'), 'utf-8');
    expect(shipContent).not.toContain('codex review --base');
    expect(shipContent).not.toContain('CODEX_REVIEWS');

    const reviewContent = fs.readFileSync(path.join(AGENTS_DIR, 'pstack-review', 'SKILL.md'), 'utf-8');
  ...
  test('hook skills have safety prose and no hooks: in frontmatter', () => {
    const HOOK_SKILLS = ['pstack-careful', 'pstack-freeze', 'pstack-guard'];
  ...
  test('Codex preamble resolves runtime assets from repo-local or global pstack roots', () => {
    // Check a skill that has a preamble (review is a good candidate)
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'pstack-review', 'SKILL.md'), 'utf-8');
  ...
  test('sidecar paths point to .agents/skills/pstack/review/ (not pstack-review/)', () => {
    // Regression: gen-skill-docs rewrote .claude/skills/review → .agents/skills/pstack-review
    // but setup puts sidecars under .agents/skills/pstack/review/. Must match setup layout.
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'pstack-review', 'SKILL.md'), 'utf-8');
    // Correct: references to sidecar files use pstack/review/ path
    expect(content).toContain('.agents/skills/pstack/review/checklist.md');
    expect(content).toContain('.agents/skills/pstack/review/design-checklist.md');
    // Wrong: must NOT reference pstack-review/checklist.md (file doesn't exist there)
    expect(content).not.toContain('.agents/skills/pstack-review/checklist.md');
    expect(content).not.toContain('.agents/skills/pstack-review/design-checklist.md');
  });

  test('sidecar paths in ship skill point to pstack/review/ for pre-landing review', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'pstack-ship', 'SKILL.md'), 'utf-8');
    // Ship references the review checklist in its pre-landing review step
    if (content.includes('checklist.md')) {
      expect(content).toContain('.agents/skills/pstack/review/');
      expect(content).not.toContain('.agents/skills/pstack-review/checklist');
    }
  });
  ...
  test('greptile-triage sidecar path is correct', () => {
    ...
    if (content.includes('greptile-triage')) {
      expect(content).toContain('.agents/skills/pstack/review/greptile-triage.md');
      expect(content).not.toContain('.agents/skills/pstack-review/greptile-triage');
    }
  });
  ...
  test('all four path rewrite rules produce correct output', () => {
    ...
    // Rule 1: ~/.claude/skills/pstack → $PSTACK_ROOT
    expect(content).not.toContain('~/.claude/skills/pstack');
    expect(content).toContain('$PSTACK_ROOT');

    // Rule 2: .claude/skills/pstack → .agents/skills/pstack
    expect(content).not.toContain('.claude/skills/pstack');

    // Rule 3: .claude/skills/review → .agents/skills/pstack/review
  ...
  test('path rewrite rules apply to all Codex skills with sidecar references', () => {
    ...
    for (const skill of CODEX_SKILLS) {
      ...
      if (content.includes('checklist.md') && !content.includes('design-checklist.md')) {
        expect(content).not.toContain('pstack-review/checklist.md');
      }
    }
  });
  ...
  test('Claude output unchanged: review skill still uses .claude/skills/ paths', () => {
    ...
    expect(content).toContain('~/.claude/skills/pstack');
  ...
  test('Claude output unchanged: all Claude skills have zero Codex paths', () => {
    for (const skill of ALL_SKILLS) {
      const content = fs.readFileSync(path.join(ROOT, skill.dir, 'SKILL.md'), 'utf-8');
      expect(content).not.toContain('~/.codex/');
      // pstack-upgrade legitimately references .agents/skills for cross-platform detection
      if (skill.dir !== 'pstack-upgrade') {
        expect(content).not.toContain('.agents/skills');
      }
    }
  });
  ...
  test('codex host produces empty outside voices in design-review', () => {
    const codexContent = fs.readFileSync(path.join(AGENTS_DIR, 'pstack-design-review', 'SKILL.md'), 'utf-8');
    expect(codexContent).not.toContain('Design Outside Voices');
  });

  test('codex host does not include Codex design block in ship', () => {
    const codexContent = fs.readFileSync(path.join(AGENTS_DIR, 'pstack-ship', 'SKILL.md'), 'utf-8');
...
describe('setup script validation', () => {
  ...
  test('Codex install uses link_codex_skill_dirs', () => {
    ...
    expect(codexSection).not.toContain('ln -snf "$PSTACK_DIR" "$CODEX_PSTACK"');
  });
  ...
  test('setup separates install path from source path for symlinked repo-local installs', () => {
    expect(setupContent).toContain('INSTALL_PSTACK_DIR=');
    expect(setupContent).toContain('SOURCE_PSTACK_DIR=');
    expect(setupContent).toContain('INSTALL_SKILLS_DIR=');
    expect(setupContent).toContain('CODEX_PSTACK="$INSTALL_PSTACK_DIR"');
    expect(setupContent).toContain('link_codex_skill_dirs "$SOURCE_PSTACK_DIR" "$CODEX_SKILLS"');
  });
  ...
  test('Codex installs always create sidecar runtime assets for the real skill target', () => {
    ...
    expect(setupContent).toContain('create_agents_sidecar "$SOURCE_PSTACK_DIR"');
  });
  ...
  test('link_codex_skill_dirs reads from .agents/skills/', () => {
    ...
    expect(fnBody).toContain('pstack*');
  });

  test('link_claude_skill_dirs creates relative symlinks', () => {
    // Claude links should be relative: ln -snf "pstack/skill_name"
    const fnStart = setupContent.indexOf('link_claude_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('linked[@]}', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('ln -snf "pstack/$skill_name"');
  });
  ...
  // T1: Sidecar skip guard — prevents .agents/skills/pstack from being linked as a skill
  test('link_codex_skill_dirs skips the pstack sidecar directory', () => {
    const fnStart = setupContent.indexOf('link_codex_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('done', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('[ "$skill_name" = "pstack" ] && continue');
  });

  // T2: Dynamic $PSTACK_ROOT paths in generated Codex preambles
  test('generated Codex preambles use dynamic PSTACK_ROOT paths', () => {
    const codexSkillDir = path.join(ROOT, '.agents', 'skills', 'pstack-ship');
    ...
    expect(content).toContain('PSTACK_ROOT=');
    expect(content).toContain('$PSTACK_BIN/');
  });
  ...
  test('setup supports --host kiro with install section and sed rewrites', () => {
    ...
    expect(setupContent).toContain('~/.kiro/skills/pstack');
  });
  ...
  test('create_codex_runtime_root exposes only runtime assets', () => {
    ...
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('pstack/SKILL.md');
    expect(fnBody).toContain('browse/dist');
    expect(fnBody).toContain('browse/bin');
    expect(fnBody).toContain('pstack-upgrade/SKILL.md');
    ...
    expect(fnBody).not.toContain('ln -snf "$pstack_dir" "$codex_pstack"');
  });

  test('direct Codex installs are migrated out of ~/.codex/skills/pstack', () => {
    expect(setupContent).toContain('migrate_direct_codex_install');
    expect(setupContent).toContain('$HOME/.pstack/repos/pstack');
    expect(setupContent).toContain('avoid duplicate skill discovery');
  });

  // --- Symlink prefix tests (PR #503) ---

  test('link_claude_skill_dirs applies pstack- prefix by default', () => {
    const fnStart = setupContent.indexOf('link_claude_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('linked[@]}', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('SKILL_PREFIX');
    expect(fnBody).toContain('link_name="pstack-$skill_name"');
  });
  ...
  test('link_claude_skill_dirs preserves already-prefixed dirs', () => {
    ...
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('linked[@]}', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    // pstack-* dirs should keep their name (e.g., pstack-upgrade stays pstack-upgrade)
    expect(fnBody).toContain('pstack-*) link_name="$skill_name"');
  });
  ...
  test('cleanup_old_claude_symlinks removes only pstack-pointing symlinks', () => {
    ...
    expect(fnBody).toContain('pstack-*) continue');
  });
  ...
  test('setup reads skill_prefix from config', () => {
    ...
    expect(setupContent).toContain('PSTACK_CONFIG');
  });
  ...
  test('cleanup_prefixed_claude_symlinks exists and uses readlink', () => {
    ...
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('readlink');
    expect(fnBody).toContain('pstack-$skill_name');
  });
  ...
  test('welcome message references SKILL_PREFIX', () => {
    // pstack-upgrade is always called pstack-upgrade (it's the actual dir name)
    // but the welcome section should exist near the prefix logic
    expect(setupContent).toContain('Run /pstack-upgrade anytime');
  });
});

describe('discover-skills hidden directory filtering', () => {
  test('discoverTemplates skips dot-prefixed directories', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pstack-discover-'));
    try {
...
describe('codex commands must not use inline $(git rev-parse --show-toplevel) for cwd', () => {
  ...
  // Use Bun.Glob to avoid ELOOP from .claude/skills/pstack symlink back to ROOT
