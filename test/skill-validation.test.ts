  ...
  test('all greptile-history write references specify both per-project and global paths', () => {
    ...
    for (const file of filesToCheck) {
      ...
      const hasBoth = (content.includes('per-project') && content.includes('global')) ||
        (content.includes('$REMOTE_SLUG/greptile-history') && content.includes('~/.pstack/greptile-history'));
...
describe('QA skill structure validation', () => {
  ...
  test('output structure references report directory layout', () => {
    ...
    expect(qaContent).toContain('.pstack/qa-reports/');
  });
});
...
describe('validate skill structure', () => {
  ...
  test('contains YC apply CTA with ref tracking', () => {
    expect(content).toContain('ycombinator.com/apply?ref=pstack');
  });
...
describe('Contributor mode preamble structure', () => {
  ...
  for (const skill of skillsWithPreamble) {
    ...
    test(`${skill} uses periodic reflection (not per-command)`, () => {
      ...
      expect(content).not.toContain('After you use pstack-provided CLIs');
    });
  }
});
...
describe('Completeness Principle in generated SKILL.md files', () => {
  ...
  test('Completeness Principle includes compression table in tier 2+ skills', () => {
    ...
    expect(content).toContain('CC+pstack');
...
describe('pstack-slug', () => {
  const SLUG_BIN = path.join(ROOT, 'bin', 'pstack-slug');
  ...
  test('eval sets variables under bash with set -euo pipefail', () => {
    const result = Bun.spawnSync(
      ['bash', '-c', 'set -euo pipefail; eval "$(./bin/pstack-slug 2>/dev/null)"; echo "SLUG=$SLUG"; echo "BRANCH=$BRANCH"'],
  ...
  test('no templates or bin scripts use source process substitution for pstack-slug', () => {
    const result = Bun.spawnSync(
      ['grep', '-r', 'source <(.*pstack-slug', '--include=*.tmpl', '--include=pstack-review-*', '.'],
      { cwd: ROOT, stdout: 'pipe', stderr: 'pipe' }
    );
...
describe('Phase 8e.5 regression test generation', () => {
  ...
  test('regression test includes full attribution comment format', () => {
    ...
    expect(content).toContain('// Report: .pstack/qa-reports/');
  });
...
describe('Codex skill', () => {
  ...
  test('codex/SKILL.md contains review log persistence', () => {
    ...
    expect(content).toContain('pstack-review-log');
  });
  ...
  test('codex-host ship/review do NOT contain adversarial review step', () => {
    ...
    const shipContent = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'pstack-ship', 'SKILL.md'), 'utf-8');
    expect(shipContent).not.toContain('codex review --base');
    expect(shipContent).not.toContain('CODEX_REVIEWS');

    const reviewContent = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'pstack-review', 'SKILL.md'), 'utf-8');
...
describe('Codex skill validation', () => {
  ...
  test('all skills (except /codex) have both Claude and Codex variants', () => {
    for (const skillDir of CLAUDE_SKILLS_WITH_TEMPLATES) {
      ...
      // Codex variant
      const codexName = skillDir.startsWith('pstack-') ? skillDir : `pstack-${skillDir}`;
  ...
  test('Codex skill names follow pstack-{name} convention', () => {
    const codexDirs = fs.readdirSync(AGENTS_DIR);
    for (const dir of codexDirs) {
      // Every directory should start with pstack
      expect(dir.startsWith('pstack')).toBe(true);
      // Root is just 'pstack', others are 'pstack-{name}'
      if (dir !== 'pstack') {
        expect(dir.startsWith('pstack-')).toBe(true);
      }
    }
...
describe('Repo mode preamble validation', () => {
  test('generated SKILL.md preamble contains REPO_MODE output', () => {
    ...
    expect(content).toContain('pstack-repo-mode');
  });
