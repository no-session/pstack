...
describeIfSelected('Office Hours Spec Review E2E', ['validate-spec-review'], () => {
  ...
  testConcurrentIfSelected('validate-spec-review', async () => {
    const result = await runSkillTest({
...
Write your summary to ${ohDir}/spec-review-summary.md`,
    ...
    recordE2E(evalCollector, '/validate-spec-review', 'Office Hours Spec Review E2E', result);
...
// Verifies that plan-eng-review writes a "## PSTACK REVIEW REPORT" section
...
`);
  ...
  test('/plan-eng-review writes PSTACK REVIEW REPORT to plan file', async () => {
    const result = await runSkillTest({
      prompt: `Read plan-eng-review/SKILL.md for the review workflow.
...
CRITICAL REQUIREMENT: plan.md IS the plan file for this review session. After completing your review, you MUST write a "## PSTACK REVIEW REPORT" section to the END of plan.md, exactly as described in the "Plan File Review Report" section of SKILL.md. If pstack-review-read is not available or returns NO_REVIEWS, write the placeholder table with all four review rows (CEO, Codex, Eng, Design). Use the Edit tool to append to plan.md — do NOT overwrite the existing plan content.
...
This review report at the bottom of the plan is the MOST IMPORTANT deliverable of this test.`,
    ...
    expect(planContent).toContain('## PSTACK REVIEW REPORT');

    // Report should be at the bottom of the file
    const reportIndex = planContent.lastIndexOf('## PSTACK REVIEW REPORT');
...
// and fallback) in validate, plan-ceo-review, plan-design-review, plan-eng-review.

describeIfSelected('Codex Offering E2E', [
  'codex-offered-validate', 'codex-offered-ceo-review',
  ...
  beforeAll(() => {
    ...
    for (const skill of ['validate', 'plan-ceo-review', 'plan-design-review', 'plan-eng-review']) {
...
Write your summary to ${testDir}/${testName}-summary.md`,
  ...
  testConcurrentIfSelected('codex-offered-validate', async () => {
    await checkCodexOffering('validate', 'codex-offered-validate', 'second opinion');
  }, 180_000);
