 ...
 * ~/.pstack/projects/$SLUG/evals/{version}-{branch}-{tier}-{timestamp}.json,
...
const LEGACY_EVAL_DIR = path.join(os.homedir(), '.pstack-dev', 'evals');

/**
 * Detect project-scoped eval dir via pstack-slug.
 * Falls back to legacy ~/.pstack-dev/evals/ if slug detection fails.
 */
export function getProjectEvalDir(): string {
  try {
    // Try repo-local pstack-slug first, then global install
    const localSlug = spawnSync('bash', ['-c', '.claude/skills/pstack/bin/pstack-slug 2>/dev/null || ~/.claude/skills/pstack/bin/pstack-slug 2>/dev/null'], {
    ...
    if (output) {
      ...
      if (slugMatch && slugMatch[1]) {
        const dir = path.join(os.homedir(), '.pstack', 'projects', slugMatch[1], 'evals');
