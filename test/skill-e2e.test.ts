...
// Eval result collector — accumulates test results, writes to ~/.pstack-dev/evals/ on finalize
...
function dumpOutcomeDiagnostic(dir: string, label: string, report: string, judgeResult: any) {
  try {
    const transcriptDir = path.join(dir, '.pstack', 'test-transcripts');
...
Report whether it worked.`,
  ...
  testIfSelected('skillmd-no-local-binary', async () => {
    // Create a tmpdir with no browse binary — no local .claude/skills/pstack/browse/dist/browse
...
Report the exact output. Do NOT try to fix or install anything — just report what you see.`,
    ...
    // On dev machines with pstack installed globally, the fallback path
    // ~/.claude/skills/pstack/browse/dist/browse exists, so we get READY.
...
OVERRIDE: Write contributor logs to ${logsDir}/ instead of ~/.pstack/contributor-logs/

Now try this browse command (it will fail — there is no binary at this path):
/nonexistent/path/browse goto https://example.com

This is a pstack issue (the browse binary is missing/misconfigured).
File a contributor report about this issue. Then tell me what you filed.`,
    ...
    expect(logContent).toContain('Hey pstack team');
  ...
  testIfSelected('session-awareness', async () => {
    ...
    const result = await runSkillTest({
      prompt: `You are running a pstack skill. The session preamble detected _SESSIONS=4 (the user has 4 pstack windows open).
...
Write your report to ${qaOnlyDir}/qa-reports/qa-only-report.md`,
    ...
    const statusLines = gitStatus.stdout.toString().trim().split('\n').filter(
      (l: string) => l.trim() && !l.includes('.prompt-tmp') && !l.includes('.pstack/') && !l.includes('qa-reports/'),
    );
...
`);
    ...
    projectDir = path.join(os.homedir(), '.pstack', 'projects', 'test-project');
  ...
  test('/plan-eng-review writes test-plan artifact to ~/.pstack/projects/', async () => {
...
describeIfSelected('pstack-upgrade E2E', ['pstack-upgrade-happy-path'], () => {
  let upgradeDir: string;
  let remoteDir: string;

  beforeAll(() => {
    upgradeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'skill-e2e-upgrade-'));
    remoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pstack-remote-'));
    ...
    // Create mock pstack install directory (local-git type)
    const mockPstack = path.join(upgradeDir, '.claude', 'skills', 'pstack');
    fs.mkdirSync(mockPstack, { recursive: true });

    // Init as a git repo
    run('git', ['init'], mockPstack);
    run('git', ['config', 'user.email', 'test@test.com'], mockPstack);
    run('git', ['config', 'user.name', 'Test'], mockPstack);

    // Create bare remote
    run('git', ['init', '--bare'], remoteDir);
    run('git', ['remote', 'add', 'origin', remoteDir], mockPstack);

    // Write old version files
    fs.writeFileSync(path.join(mockPstack, 'VERSION'), '0.5.0\n');
    fs.writeFileSync(path.join(mockPstack, 'CHANGELOG.md'),
      '# Changelog\n\n## 0.5.0 — 2026-03-01\n\n- Initial release\n');
    fs.writeFileSync(path.join(mockPstack, 'setup'),
      '#!/bin/bash\necho "Setup completed"\n', { mode: 0o755 });

    // Initial commit + push
    run('git', ['add', '.'], mockPstack);
    run('git', ['commit', '-m', 'initial'], mockPstack);
    run('git', ['push', '-u', 'origin', 'HEAD:main'], mockPstack);

    // Create new version (simulate upstream release)
    fs.writeFileSync(path.join(mockPstack, 'VERSION'), '0.6.0\n');
    fs.writeFileSync(path.join(mockPstack, 'CHANGELOG.md'),
      '# Changelog\n\n## 0.6.0 — 2026-03-15\n\n- New feature: interactive design review\n- Fix: snapshot flag validation\n\n## 0.5.0 — 2026-03-01\n\n- Initial release\n');
    run('git', ['add', '.'], mockPstack);
    run('git', ['commit', '-m', 'release 0.6.0'], mockPstack);
    run('git', ['push', 'origin', 'HEAD:main'], mockPstack);

    // Reset working copy back to old version
    run('git', ['reset', '--hard', 'HEAD~1'], mockPstack);

    // Copy pstack-upgrade skill
    fs.mkdirSync(path.join(upgradeDir, 'pstack-upgrade'), { recursive: true });
    fs.copyFileSync(
      path.join(ROOT, 'pstack-upgrade', 'SKILL.md'),
      path.join(upgradeDir, 'pstack-upgrade', 'SKILL.md'),
    );
  ...
  testIfSelected('pstack-upgrade-happy-path', async () => {
    const mockPstack = path.join(upgradeDir, '.claude', 'skills', 'pstack');
    const result = await runSkillTest({
      prompt: `Read pstack-upgrade/SKILL.md for the upgrade workflow.

You are running /pstack-upgrade standalone. The pstack installation is at ./.claude/skills/pstack (local-git type — it has a .git directory with an origin remote).
...
IMPORTANT: The install directory is at ./.claude/skills/pstack — use that exact path.`,
      workingDirectory: upgradeDir,
      maxTurns: 20,
      timeout: 180_000,
      testName: 'pstack-upgrade-happy-path',
      runId,
    });

    logCost('/pstack-upgrade happy path', result);

    // Check that the version was updated
    const versionAfter = fs.readFileSync(path.join(mockPstack, 'VERSION'), 'utf-8').trim();
    const output = result.output || '';
    ...
    recordE2E('/pstack-upgrade happy path', 'pstack-upgrade E2E', result, {
