...
const scriptPath = join(import.meta.dir, "..", "bin", "pstack-global-discover.ts");

describe("pstack-global-discover", () => {
  describe("normalizeRemoteUrl", () => {
    // Dynamically import to test the exported function
    let normalizeRemoteUrl: (url: string) => string;

    beforeEach(async () => {
      const mod = await import("../bin/pstack-global-discover.ts");
    ...
    test("SSH and HTTPS for same repo normalize to same URL", () => {
      const ssh = normalizeRemoteUrl("git@github.com:pawan/pstack.git");
      const https = normalizeRemoteUrl("https://github.com/pawan/pstack.git");
      const httpsNoDotGit = normalizeRemoteUrl("https://github.com/pawan/pstack");
