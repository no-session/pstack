...
echo '{"skill":"qa","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.pstack/analytics/skill-usage.jsonl 2>/dev/null || true
...
| Mode | full | `--regression .pstack/qa-reports/baseline.json` |
| Output dir | `.pstack/qa-reports/` | `Output to /tmp/qa` |
...
```bash
mkdir -p .pstack/qa-reports/screenshots
```
...
1. **Project-scoped test plans:** Check `~/.pstack/projects/` for recent `*-test-plan-*.md` files for this repo
   ```bash
   setopt +o nomatch 2>/dev/null || true  # zsh compat
   eval "$(~/.claude/skills/pstack/bin/pstack-slug 2>/dev/null)"
   ls -t ~/.pstack/projects/$SLUG/*-test-plan-*.md 2>/dev/null | head -1
   ```
...
- Include full attribution comment:
  ...
  // Report: .pstack/qa-reports/qa-report-{domain}-{date}.md
  ```
...
**Local:** `.pstack/qa-reports/qa-report-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:** Write test outcome artifact for cross-session context:
```bash
eval "$(~/.claude/skills/pstack/bin/pstack-slug 2>/dev/null)" && mkdir -p ~/.pstack/projects/$SLUG
```
Write to `~/.pstack/projects/{slug}/{user}-{branch}-test-outcome-{datetime}.md`
