...
bun run eval:list    # list all eval runs from ~/.pstack-dev/evals/
...
--verbose`). Results are persisted to `~/.pstack-dev/evals/` with auto-comparison
...
2. **If missing, AskUserQuestion** — let the user tell you or let pstack search the repo
3. **Persist the answer to CLAUDE.md** so we never have to ask again

This applies to test commands, eval commands, deploy commands, and any other
project-specific behavior. The project owns its config; pstack reads it.
...
When developing pstack, `.claude/skills/pstack` may be a symlink back to this
working directory (gitignored). This means skill changes are **live immediately** —
great for rapid iteration, risky during big refactors where half-written skills
could break other Claude Code sessions using pstack concurrently.

**Check once per session:** Run `ls -la .claude/skills/pstack` to see if it's a
symlink or a real copy. If it's a symlink to your working directory, be aware that:
- Template changes + `bun run gen:skill-docs` immediately affect all pstack invocations
- Breaking changes to SKILL.md.tmpl files can break concurrent pstack sessions
- During large refactors, remove the symlink (`rm .claude/skills/pstack`) so the
  global install at `~/.claude/skills/pstack/` is used instead

**Prefix setting:** Skill symlinks use either short names (`qa -> pstack/qa`) or
namespaced (`pstack-qa -> pstack/qa`), controlled by `skill_prefix` in
`~/.pstack/config.yaml`. When vendoring into a project, run `./setup` after
...
before going live (especially if the user is actively using pstack in other windows).
...
When estimating or discussing effort, always show both human-team and CC+pstack time:
...
Contributors can store long-range vision docs and design documents in `~/.pstack-dev/plans/`.
...
1. Push your branch
2. Fetch and reset in the skill directory: `cd ~/.claude/skills/pstack && git fetch origin && git reset --hard origin/main`
3. Rebuild: `cd ~/.claude/skills/pstack && bun run build`

Or copy the binaries directly:
- `cp browse/dist/browse ~/.claude/skills/pstack/browse/dist/browse`
- `cp design/dist/design ~/.claude/skills/pstack/design/dist/design`
