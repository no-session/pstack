 ...
 *
 * Resolution order:
 * 1. ~/.pstack/openai.json → { "api_key": "sk-..." }
...
const CONFIG_PATH = path.join(process.env.HOME || "~", ".pstack", "openai.json");

export function resolveApiKey(): string | null {
  // 1. Check ~/.pstack/openai.json
  try {
...
/**
 * Save an API key to ~/.pstack/openai.json with 0600 permissions.
 */
...
export function requireApiKey(): string {
  ...
  if (!key) {
    ...
    console.error("  or save to ~/.pstack/openai.json: { \"api_key\": \"sk-...\" }");
