...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.claude/skills/pstack/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
PROJECT_HISTORY="$HOME/.pstack/projects/$REMOTE_SLUG/greptile-history.md"
```
...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.claude/skills/pstack/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
mkdir -p "$HOME/.pstack/projects/$REMOTE_SLUG"
mkdir -p ~/.pstack
```
...
Example entries:
```
2026-03-13 | pawan/myapp | fp | app/services/auth_service.rb | race-condition
2026-03-13 | pawan/myapp | fix | app/models/user.rb | null-check
2026-03-13 | pawan/myapp | already-fixed | lib/payments.rb | error-handling
```
