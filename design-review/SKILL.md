...
echo '{"skill":"design-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.pstack/analytics/skill-usage.jsonl 2>/dev/null || true
...
**Find the pstack designer (optional — enables target mockup generation):**
...
REPORT_DIR=~/.pstack/projects/$SLUG/designs/design-audit-$(date +%Y%m%d)
...
**Local:** `.pstack/design-reports/design-audit-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:**
```bash
eval "$(~/.claude/skills/pstack/bin/pstack-slug 2>/dev/null)" && mkdir -p ~/.pstack/projects/$SLUG
```
Write to: `~/.pstack/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md`
...
```
~/.pstack/projects/$SLUG/designs/design-audit-{YYYYMMDD}/
...
If the pstack designer is available and the finding involves visual layout, hierarchy, or spacing (not just a CSS value fix like wrong color or font-size), generate a target mockup showing what the corrected version should look like:
...
eval "$(~/.claude/skills/pstack/bin/pstack-slug 2>/dev/null)" && mkdir -p ~/.pstack/projects/$SLUG
```
Write a one-line summary to `~/.pstack/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md` with a pointer to the full report in `$REPORT_DIR`.
