...
version: 2.0.0
description: |
  Weekly engineering retrospective for solo builders. Analyzes commit history,
  work patterns, code quality metrics, and shipping velocity.
  Solo-aware: focuses on your personal output, shipping streaks, and momentum.
  Also works for small teams.
...
echo '{"skill":"retro","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.pstack/analytics/skill-usage.jsonl 2>/dev/null || true
...
Generates a comprehensive engineering retrospective analyzing commit history, work patterns, and code quality metrics. Solo-first: focuses on your shipping velocity, consistency, and output. When multiple contributors exist, includes team breakdowns. Designed for a solo founder using AI as a force multiplier.
...
```
  EUREKA /office-hours (branch: pawan/auth-rethink): "Session tokens don't need server storage — browser crypto API makes client-side JWT validation viable"
...
```json
{
  ...
  "authors": {
    "Builder": { "commits": 32, "insertions": 2400, "deletions": 300, "test_ratio": 0.41, "top_area": "browse/" },
...
- Top Work: 3 bullet points summarizing the user's major themes, inferred from
  ...
  not "feat: pstack-global-discover" + "feat: /retro global template".
