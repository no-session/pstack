...
version: 1.0.0
description: |
  Ship workflow: detect + merge base branch, run tests, review diff, bump VERSION, update CHANGELOG, commit, push, deploy. Use when asked to "ship", "deploy", "push to main", "create a PR", or "merge and push".
...
Check diff size: `git diff <base>...HEAD --stat | tail -1`. If the diff is >200 lines, add: "Note: This is a large diff. Consider running `/plan-eng-review` or `/plan` for architecture-level review before shipping."
...
Log coverage and plan completion data so `/reflect` can track trends:
