...
version: 1.0.0
description: |
  Ship workflow for solo founders. Two modes: (1) Solo mode (default for REPO_MODE=solo): run tests, bump VERSION, update CHANGELOG, commit, push to main, deploy. No PR. (2) PR mode: same flow but creates a PR for review. Use when asked to "ship", "deploy", "push to main", "create a PR", or "merge and push". Proactively suggest when the user says code is ready.
...
echo '{"skill":"ship","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.pstack/analytics/skill-usage.jsonl 2>/dev/null || true
...
For Design Review: run `source <(~/.claude/skills/pstack/bin/pstack-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 3.5, but consider running /design-review for a full visual audit post-implementation." Still never block.
...
[ -f .pstack/no-test-bootstrap ] && echo "BOOTSTRAP_DECLINED"
```
...
If user picks H → write `.pstack/no-test-bootstrap` and continue without tests.
...
If user picks C → write `.pstack/no-test-bootstrap`. Tell user: "If you change your mind later, delete `.pstack/no-test-bootstrap` and re-run." Continue without tests.
...
- Create an issue assigned to that person (use the platform detected in Step 0):
  - **If GitHub:**
    ...
    gh issue create \
      ...
      --body "Found failing on branch <current-branch>. Failure is pre-existing.\n\n**Error:**\n```\n<first 10 lines>\n```\n\n**Last modified by:** <author>\n**Noticed by:** pstack /ship on <date>" \
...
Write to `~/.pstack/projects/{slug}/{user}-{branch}-ship-test-plan-{datetime}.md`:
...
# Compute project slug for ~/.pstack/projects/ lookup
_PLAN_SLUG=$(git remote get-url origin 2>/dev/null | sed 's|.*[:/]\([^/]*/[^/]*\)\.git$|\1|;s|.*[:/]\([^/]*/[^/]*\)$|\1|' | tr '/' '-' | tr -cd 'a-zA-Z0-9._-') || true
_PLAN_SLUG="${_PLAN_SLUG:-$(basename "$PWD" | tr -cd 'a-zA-Z0-9._-')}"
# Search common plan file locations (project designs first, then personal/local)
for PLAN_DIR in "$HOME/.pstack/projects/$_PLAN_SLUG" "$HOME/.claude/plans" "$HOME/.codex/plans" ".pstack/plans"; do
  [ -d "$PLAN_DIR" ] || continue
...
- Review report sections (`## PSTACK REVIEW REPORT`)
...
Check if the diff touches frontend files using `pstack-diff-scope`:

```bash
source <(~/.claude/skills/pstack/bin/pstack-diff-scope <base> 2>/dev/null)
```
...
```bash
~/.claude/skills/pstack/bin/pstack-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
```bash
~/.claude/skills/pstack/bin/pstack-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"commit":"'"$(git rev-parse --short HEAD)"'","via":"ship"}'
```
...
which codex 2>/dev/null && echo "CODEX_AVAILABLE" || echo "CODEX_NOT_AVAILABLE"
# Respect old opt-out
OLD_CFG=$(~/.claude/skills/pstack/bin/pstack-config get codex_reviews 2>/dev/null || true)
...
```bash
~/.claude/skills/pstack/bin/pstack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"medium","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
~/.claude/skills/pstack/bin/pstack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"large","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
- Message: "pstack recommends maintaining a TODOS.md organized by skill/component, then priority (P0 at top through P4, then Completed at bottom). See TODOS-format.md for the full format. Would you like to create one?"
...
## Step 8: Push and Ship

**Solo mode** (REPO_MODE=solo or user is on main branch with no upstream PR):
Skip PR creation entirely. The code is already pushed to main. Output:

```
SHIPPED to main. Commits pushed. Deploy should auto-trigger.
If no auto-deploy: run your deploy command (e.g., `vercel --prod`, `railway up`, `fly deploy`).
```

Then proceed to Step 8.5.

**PR mode** (REPO_MODE=collaborative, or user explicitly asked for a PR, or on a feature branch):
...
🤖 Shipped with [pstack](https://github.com/pawan/pstack)
```
...
Append to `~/.pstack/projects/$SLUG/$BRANCH-reviews.jsonl`:

```bash
echo '{"skill":"ship","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","coverage_pct":COVERAGE_PCT,"plan_items_total":PLAN_TOTAL,"plan_items_done":PLAN_DONE,"verification_result":"VERIFY_RESULT","version":"VERSION","branch":"BRANCH"}' >> ~/.pstack/projects/$SLUG/$BRANCH-reviews.jsonl
```
