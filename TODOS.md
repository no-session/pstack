# pstack TODOs

## Phase 1 — Foundation (current)
- [x] Fork gstack, rename all references to pstack
- [x] Write bootstrapper-focused ETHOS.md
- [x] Write new README.md
- [x] Clean package.json, CHANGELOG, VERSION
- [ ] Adapt /validate for solo founder mode (remove YC-specific framing)
- [ ] Adapt /ship for push-to-main workflow (no PR by default)
- [ ] Adapt /plan-ceo-review to focus on revenue/market validation
- [ ] Adapt /reflect for solo developer (remove team breakdowns, add revenue tracking)
- [ ] Test setup script works end-to-end
- [ ] Build binaries with bun

## Phase 2 — New Skills
- [ ] `/validate` — willingness-to-pay validation before building
- [ ] `/launch` — landing page + waitlist + payment setup pipeline
- [ ] `/monetize` — Stripe/LemonSqueezy/Gumroad integration in one command
- [ ] `/seo` — basic SEO setup from day 1 (meta tags, sitemap, OG images)
- [ ] `/cost-check` — audit infrastructure costs, flag anything over $20/mo

## Phase 3 — Polish
- [ ] Update ARCHITECTURE.md for pstack changes
- [ ] Write CONTRIBUTING.md for pstack contributors
- [ ] Add pstack-specific test fixtures
- [ ] Update README with full 29-skill table
