/**
 * Tests for bin/pstack-config bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * PSTACK_STATE_DIR env override for full isolation.
 */
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'pstack-config');
...
beforeEach(() => {
  stateDir = mkdtempSync(join(tmpdir(), 'pstack-config-test-'));
});
...
describe('pstack-config', () => {
  ...
  test('set creates state dir if missing', () => {
    ...
    const { exitCode } = run(['set', 'foo', 'bar'], { PSTACK_STATE_DIR: nestedDir });
    expect(exitCode).toBe(0);
