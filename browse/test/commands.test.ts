...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync('/tmp/pstack-cli-');
    const execPath = path.join(root, '.claude/skills/pstack/browse/dist/browse');
    const serverPath = path.join(root, '.claude/skills/pstack/browse/src/server.ts');
