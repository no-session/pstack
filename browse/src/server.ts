/**
 * pstack browse server — persistent Chromium daemon
 *
 * Architecture:
 ...
 *
 * State:
 *   State file: <project-root>/.pstack/browse.json (set via BROWSE_STATE_FILE env)
 *   Log files:  <project-root>/.pstack/browse-{console,network,dialog}.log
...
function generateHelpText(): string {
  ...
  const lines = ['pstack browse — headless browser for AI agents', '', 'Commands:'];
...
const SESSIONS_DIR = path.join(process.env.HOME || '/tmp', '.pstack', 'sidebar-sessions');
...
function findBrowseBin(): string {
  const candidates = [
    ...
    path.resolve(__dirname, '..', '..', '.claude', 'skills', 'pstack', 'browse', 'dist', 'browse'),
    path.join(process.env.HOME || '', '.claude', 'skills', 'pstack', 'browse', 'dist', 'browse'),
  ];
...
function shortenPath(str: string): string {
  return str
    ...
    .replace(/\.claude\/skills\/pstack\//g, '')
...
function createWorktree(sessionId: string): string | null {
  try {
    ...
    const worktreeDir = path.join(process.env.HOME || '/tmp', '.pstack', 'worktrees', sessionId.slice(0, 8));
...
function spawnClaude(userMessage: string, extensionUrl?: string | null): void {
  ...
  const agentQueue = process.env.SIDEBAR_QUEUE_PATH || path.join(process.env.HOME || '/tmp', '.pstack', 'sidebar-agent-queue.jsonl');
  const pstackDir = path.dirname(agentQueue);
  ...
  try {
    fs.mkdirSync(pstackDir, { recursive: true });
...
async function shutdown() {
  ...
  const profileDir = path.join(process.env.HOME || '/tmp', '.pstack', 'chromium-profile');
