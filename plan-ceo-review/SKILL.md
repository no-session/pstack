...
version: 1.0.0
description: |
  Founder-mode for solo builders plan review. Rethink the problem, find the 10-star product,
...
SLUG=$(~/.claude/skills/pstack/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-' || echo 'no-branch')
DESIGN=$(ls -t ~/.pstack/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t ~/.pstack/projects/$SLUG/*-design-*.md 2>/dev/null | head -1)
[ -n "$DESIGN" ] && echo "Design doc found: $DESIGN" || echo "No design doc found"
```
If a design doc exists (from `/validate`), read it. Use it as the source of truth for the problem statement, constraints, and chosen approach. If it has a `Supersedes:` field, note that this is a revised design.

**Handoff note check** (reuses $SLUG and $BRANCH from the design doc check above):
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
HANDOFF=$(ls -t ~/.pstack/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null | head -1)
...
from a prior CEO review session that paused so the user could run `/validate`. Use it
...
> "No design doc found for this branch. `/validate` produces a structured problem
> statement, premise challenge, and explored alternatives — it gives this review much
> sharper input to work with. Takes about 10 minutes. The design doc is per-feature,
> not per-product — it captures the thinking behind this specific change."

Options:
- A) Run /validate now (we'll pick up the review right after)
- B) Skip — proceed with standard review

If they skip: "No worries — standard review. If you ever want sharper input, try
/validate first next time." Then proceed normally. Do not re-offer later in the session.

If they choose A:

Say: "Running /validate inline. Once the design doc is ready, I'll pick up
the review right where we left off."

Read the validate skill file from disk using the Read tool:
`~/.claude/skills/pstack/validate/SKILL.md`
...
"Could not load /validate — proceeding with standard review."

After /validate completes, re-run the design doc check:
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
SLUG=$(~/.claude/skills/pstack/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-' || echo 'no-branch')
...
sure," or is clearly exploring rather than reviewing — offer `/validate`:

> "It sounds like you're still figuring out what to build — that's totally fine, but
> that's what /validate is designed for. Want to run /validate right now?
> We'll pick up right where we left off."

Options: A) Yes, run /validate now. B) No, keep going.
If they keep going, proceed normally — no guilt, no re-asking.

If they choose A: Read the validate skill file from disk:
`~/.claude/skills/pstack/validate/SKILL.md`
...
```bash
eval "$(~/.claude/skills/pstack/bin/pstack-slug 2>/dev/null)" && mkdir -p ~/.pstack/projects/$SLUG/ceo-plans
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p ~/.pstack/projects/$SLUG/ceo-plans/archive
# For each stale plan: mv ~/.pstack/projects/$SLUG/ceo-plans/{old-plan}.md ~/.pstack/projects/$SLUG/ceo-plans/archive/
```

Write to `~/.pstack/projects/$SLUG/ceo-plans/{date}-{feature-slug}.md` using this format:
...
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.pstack/analytics/spec-review.jsonl 2>/dev/null || true
```
...
```
NOTE: These represent human-team implementation hours. With CC + pstack,
...
**Persist the result:**
```bash
~/.claude/skills/pstack/bin/pstack-review-log '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
* **Effort estimate:** S/M/L/XL (human team) → with CC+pstack: S→S, M→S, L→M, XL→L
* **Priority:** P1/P2/P3
...
rm -f ~/.pstack/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null || true
```
...
`~/.pstack/` (user config directory, not project files). The skill preamble
already writes to `~/.pstack/sessions/` and `~/.pstack/analytics/` — this is
the same pattern. The review dashboard depends on this data. Skipping this
command breaks the review readiness dashboard in /ship.

```bash
~/.claude/skills/pstack/bin/pstack-review-log '{"skill":"plan-ceo-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","scope_proposed":N,"scope_accepted":N,"scope_deferred":N,"commit":"COMMIT"}'
```
...
**Source attribution:** If the most recent entry for a skill has a \`"via"\` field, append it to the status label in parentheses. Examples: `plan-eng-review` with `via:"plan"` shows as "CLEAR (PLAN via /plan)". `review` with `via:"ship"` shows as "CLEAR (DIFF via /ship)". Entries without a `via` field show as "CLEAR (PLAN)" or "CLEAR (DIFF)" as before.

Note: `plan-voices` and `design-outside-voices` entries are audit-trail-only (forensic data for cross-model consensus analysis). They do not appear in the dashboard and are not checked by any consumer.
...
**Review tiers:**
- **Eng Review (required by default):** The only review that gates shipping. Covers architecture, code quality, tests, performance. Can be disabled globally with \`pstack-config set skip_eng_review true\` (the "don't bother me" setting).
...
- Search the plan file for a \`## PSTACK REVIEW REPORT\` section **anywhere** in the file
  (not just at the end — content may have been added after it).
- If found, **replace it** entirely using the Edit tool. Match from \`## PSTACK REVIEW REPORT\`
...
- **B)** Keep in `~/.pstack/projects/` only (local, personal reference)
- **C)** Skip
