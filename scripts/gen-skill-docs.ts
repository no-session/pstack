...
/** pstack's 10 AI slop anti-patterns — shared between DESIGN_METHODOLOGY and DESIGN_HARD_RULES */
...
function codexSkillName(skillDir: string): string {
  if (skillDir === '.' || skillDir === '') return 'pstack';
  // Don't double-prefix: pstack-upgrade → pstack-upgrade (not pstack-pstack-upgrade)
  if (skillDir.startsWith('pstack-')) return skillDir;
  return `pstack-${skillDir}`;
}
...
function processTemplate(tmplPath: string, host: Host = 'claude'): { outputPath: string; content: string; symlinkLoop?: boolean } {
  ...
  if (host === 'codex') {
    ...
    // Guard against symlink loops: if .agents/skills/pstack → repo root,
    // writing to .agents/skills/pstack/SKILL.md would overwrite the Claude version.
  ...
  if (host === 'codex') {
    ...
    content = content.replace(/~\/\.claude\/skills\/pstack/g, ctx.paths.skillRoot);
    content = content.replace(/\.claude\/skills\/pstack/g, ctx.paths.localSkillRoot);
    content = content.replace(/\.claude\/skills\/review/g, '.agents/skills/pstack/review');
