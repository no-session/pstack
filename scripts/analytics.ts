#!/usr/bin/env bun
/**
 * analytics — CLI for viewing pstack skill usage statistics.
 *
 * Reads ~/.pstack/analytics/skill-usage.jsonl and displays:
...
const ANALYTICS_FILE = path.join(os.homedir(), '.pstack', 'analytics', 'skill-usage.jsonl');
...
export function formatReport(events: AnalyticsEvent[], period: string = 'all'): string {
  ...
  const lines: string[] = [];
  lines.push('pstack skill usage analytics');
