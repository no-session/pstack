...
PSTACK_ROOT="$HOME/.codex/skills/pstack"
[ -n "$_ROOT" ] && [ -d "$_ROOT/.agents/skills/pstack" ] && PSTACK_ROOT="$_ROOT/.agents/skills/pstack"
PSTACK_BIN="$PSTACK_ROOT/bin"
PSTACK_BROWSE="$PSTACK_ROOT/browse/dist"
`
    : '';
...
\`\`\`bash
${runtimeRoot}_UPD=$(${ctx.paths.binDir}/pstack-update-check 2>/dev/null || ${ctx.paths.localSkillRoot}/bin/pstack-update-check 2>/dev/null || true)
...
_CONTRIB=$(${ctx.paths.binDir}/pstack-config get pstack_contributor 2>/dev/null || true)
_PROACTIVE=$(${ctx.paths.binDir}/pstack-config get proactive 2>/dev/null || echo "true")
_PROACTIVE_PROMPTED=$([ -f ~/.pstack/.proactive-prompted ] && echo "yes" || echo "no")
_BRANCH=$(git branch --show-current 2>/dev/null || echo "unknown")
echo "BRANCH: $_BRANCH"
_SKILL_PREFIX=$(${ctx.paths.binDir}/pstack-config get skill_prefix 2>/dev/null || echo "false")
echo "PROACTIVE: $_PROACTIVE"
echo "PROACTIVE_PROMPTED: $_PROACTIVE_PROMPTED"
echo "SKILL_PREFIX: $_SKILL_PREFIX"
source <(${ctx.paths.binDir}/pstack-repo-mode 2>/dev/null) || true
...
function generateUpgradeCheck(ctx: TemplateContext): string {
  return `If \`PROACTIVE\` is \`"false"\`, do not proactively suggest pstack skills AND do not
...
If A: run \`${ctx.paths.binDir}/pstack-config set proactive true\`
If B: run \`${ctx.paths.binDir}/pstack-config set proactive false\`
