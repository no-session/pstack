 ...
 *
 * Review logs are stored locally at ~/.pstack/reviews/review-log.jsonl.
...
echo '{"skill":"${_ctx.skillName}","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.pstack/analytics/spec-review.jsonl 2>/dev/null || true
\`\`\`
...
\`~/.claude/skills/pstack/${first}/SKILL.md\`
