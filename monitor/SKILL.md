...
version: 1.0.0
description: |
  Post-deploy monitor monitoring. Watches the live app for console errors,
  performance regressions, and page failures using the browse daemon. Takes
  periodic screenshots, compares against pre-deploy baselines, and alerts
  on anomalies. Use when: "monitor deploy", "monitor", "post-deploy check",
...
# /monitor — Post-Deploy Visual Monitor
...
## User-invocable
When the user types `/monitor`, run this skill.

## Arguments
- `/monitor <url>` — monitor a URL for 10 minutes after deploy
- `/monitor <url> --duration 5m` — custom monitoring duration (1m to 30m)
- `/monitor <url> --baseline` — capture baseline screenshots (run BEFORE deploying)
- `/monitor <url> --pages /,/dashboard,/settings` — specify pages to monitor
- `/monitor <url> --quick` — single-pass health check (no continuous monitoring)
...
```bash
eval "$(~/.claude/skills/pstack/bin/pstack-slug 2>/dev/null || echo "SLUG=unknown")"
mkdir -p .pstack/monitor-reports
mkdir -p .pstack/monitor-reports/baselines
mkdir -p .pstack/monitor-reports/screenshots
```
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".pstack/monitor-reports/baselines/<page-name>.png"
$B console --errors
$B perf
$B text
```
...
Save the baseline manifest to `.pstack/monitor-reports/baseline.json`:
...
Then STOP and tell the user: "Baseline captured. Deploy your changes, then run `/monitor <url>` to monitor."
...
- **Question:** Which pages should the monitor monitor?
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".pstack/monitor-reports/screenshots/pre-<page-name>.png"
$B console --errors
$B perf
```
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".pstack/monitor-reports/screenshots/<page-name>-<check-number>.png"
$B console --errors
$B perf
```
...
Screenshots:   .pstack/monitor-reports/screenshots/

VERDICT: [DEPLOY IS HEALTHY / DEPLOY HAS ISSUES — details above]
```

Save report to `.pstack/monitor-reports/{date}-monitor.md` and `.pstack/monitor-reports/{date}-monitor.json`.

Log the result for the review dashboard:

```bash
eval "$(~/.claude/skills/pstack/bin/pstack-slug 2>/dev/null)"
mkdir -p ~/.pstack/projects/$SLUG
```

Write a JSONL entry: `{"skill":"monitor","timestamp":"<ISO>","status":"<HEALTHY/DEGRADED/BROKEN>","url":"<url>","duration_min":<N>,"alerts":<N>}`
...
- **Baseline is king.** Without a baseline, monitor is a health check. Encourage `--baseline` before deploying.
