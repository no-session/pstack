...
echo '{"skill":"benchmark","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.pstack/analytics/skill-usage.jsonl 2>/dev/null || true
...
mkdir -p .pstack/benchmark-reports
mkdir -p .pstack/benchmark-reports/baselines
```
...
Write to `.pstack/benchmark-reports/baselines/baseline.json`.
...
Write to `.pstack/benchmark-reports/{date}-benchmark.md` and `.pstack/benchmark-reports/{date}-benchmark.json`.
