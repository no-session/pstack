...
If `PROACTIVE` is `false`: do NOT proactively suggest other pstack skills during this session.
...
- Upgrades → /pstack-upgrade

If the user opts out of suggestions, run `pstack-config set proactive false`.
If they opt back in, run `pstack-config set proactive true`.

# pstack browse: QA Testing & Dogfooding
