// pstack telemetry-ingest edge function
// Validates and inserts a batch of telemetry events.
// Called by bin/pstack-telemetry-sync.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const event of events) {
      // Required fields
      if (!event.ts || !event.pstack_version || !event.os || !event.outcome) {
      ...
      rows.push({
        ...
        pstack_version: String(event.pstack_version).slice(0, 20),
